...
/** @see mindustry.maps.planet.SerpuloPlanetGenerator */
public class LysettaPlanetGenerator extends mindustry.maps.generators.PlanetGenerator{

    //alternate, less direct generation
    final boolean indirectPaths = true;
    //random water patches
    final boolean genLakes = true;
    
    final float metalDstScl = 0.25f;
    final float heightYOffset = 42.7f;
    final float scl = 5f;
    final float waterOffset = 0.04f;
    final float heightScl = 1.01f;
    ...
    final ObjectMap<Block, Block> dec = ObjectMap.of(
        Blocks.sporeMoss, Blocks.sporeCluster, Blocks.moss, Blocks.sporeCluster,
        Blocks.taintedWater, Blocks.water, Blocks.darksandTaintedWater, Blocks.darksandWater
    );
    final ObjectMap<Block, Block> tars = ObjectMap.of(
        Blocks.sporeMoss, Blocks.shale, Blocks.moss, Blocks.shale
    );

    final float water = 2.0f / arr[0].length;
    final Vec3 basePos = new Vec3(0.9341721f, 0.0f, 0.3568221f);
    ...
    public boolean allowLanding(Sector sector){
        return sector.planet.allowLaunchToNumbered && (sector.hasBase() || sector.near().contains(s -> s.hasBase() && (s.info.bestCoreType.size >= 4 || s.isBeingPlayed() && state.rules.defaultTeam.cores().contains(b -> b.block.size >= 4))));
    }
    ...
    public void getEmissiveColor(Vec3 position, Color out){
        ...
        Object[] sectors = FRPlanets.lysetta.sectors.items;
        int size = FRPlanets.lysetta.sectors.size;
        ...
        final float freq = 0.05f;
        if(position.dst(basePos) < 0.55f
            ? dst * metalDstScl + Simplex.noise3d(seed + 1, 3, 0.4, 5.5f, position.x, position.y + 200f, position.z) * 0.08f + ((basePos.dst(position) + 0.00f) % freq < freq / 2f ? 1f : 0f) * 0.07f < 0.08f
            : dst * metalDstScl + Simplex.noise3d(seed, 3, 0.4, 9f, position.x, position.y + 370f, position.z) * 0.06f < 0.045f
        ){
            out.set(Team.crux.color).mul(0.8f + Simplex.noise3d(seed, 1, 1, 9f, position.x, position.y + 99f, position.z) * 0.4f).lerp(Team.sharded.color, 0.2f * Simplex.noise3d(seed, 1, 1, 9f, position.x, position.y + 999f, position.z)).toFloatBits();
        }else if(captureDst * metalDstScl + Simplex.noise3d(seed, 3, 0.4, 9f, position.x, position.y + 600f, position.z) * 0.07f < 0.05 * lightScl){
            out.set(Team.sharded.color).mul(0.7f + Simplex.noise3d(seed, 1, 1, 9f, position.x, position.y + 99f, position.z) * 0.4f).lerp(Team.crux.color, 0.3f * Simplex.noise3d(seed, 1, 1, 9f, position.x, position.y + 999f, position.z)).toFloatBits();
        }
    }
    ...
    Block getBlock(Vec3 position){
        float height = Mathf.clamp(rawHeight(position) * 1.2f);
        ...
        float temp = Mathf.lerp(Mathf.clamp(Math.abs(py * 2f) / scl), tnoise, 0.5f);

        var res = arr[Mathf.clamp((int)(temp * arr.length), 0, arr[0].length - 1)][Mathf.clamp((int)(height * arr[0].length), 0, arr[0].length - 1)];
        if(Simplex.noise3d(seed, 4, 0.55f, 1f / 2f, px, py + 999f, pz) * 0.3f + position.dst(0, 0, 1f) * 0.2f > 0.4f){
    ...
    protected void generate(){
        class Room{
            ...
            void connect(Room to){
                ...
                var midpoint = Tmp.v1.set(to.x, to.y).add(x, y).scl(0.5f);
                rand.nextFloat();
            ...
            void joinLiquid(int x1, int y1, int x2, int y2){
                ...
                path.each(t -> {
                    ...
                    if(Mathf.dst2(t.x, t.y, x2, y2) <= avoid * avoid) return;
        ...
        final int waterCheckRad = 5;
        for(int i = 0; i < 360; i += angleStep){
            int angle = offset + i;
            int cx = (int)(width / 2.0f + Angles.trnsx(angle, length));
            int cy = (int)(height / 2.0f + Angles.trnsy(angle, length));
            ...
            if(waterTiles <= 4 || (i + angleStep >= 360)){
                ...
                for(int j = 0; j < enemySpawns; j++){
                    ...
                    Tmp.v1.set(cx - width / 2.0f, cy - height / 2.0f).rotate(180f + enemyOffset).add(width / 2.0f, height / 2.0f);
        ...
        pass((x, y) -> {
            ...
            if(value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)){
                ...
                if(!(floor == Blocks.ice || floor == Blocks.iceSnow || floor == Blocks.snow || floor.asFloor().isLiquid)){
                    floor = spore ? (deep ? Blocks.taintedWater : Blocks.darksandTaintedWater) : (deep ? Blocks.water : (floor == Blocks.sand || floor == Blocks.salt ? Blocks.sandWater : Blocks.darksandWater));
                }
            }
        });
        ...
        final float poles = Math.abs(sector.tile.v.y),
        nmag = 0.5f, scl = 1f, addscl = 1.4f;
        ...
        pass((x, y) -> {
            ...
            if(floor == Blocks.hotrock){
                if(Math.abs(0.5f - noise(x - 90, y, 4, 0.8, 80)) > 0.035f){
                    floor = Blocks.basalt;
                }else{
                    ore = Blocks.air;
                    boolean all = true;
            ...
            if(rand.chance(0.0075)){
                ...
                for(var p : Geometry.d4){
                    var other = tiles.get(x + p.x, y + p.y);
        ...
        if(ruinCount > 0){
            ...
            for(int i = 0; i < ints.size && placed < ruinCount; i++){
                ...
                if(part != null && BaseGenerator.tryPlace(part, x, y, Team.derelict, rand, (cx, cy) -> {
                    var other = tiles.getn(cx, cy);
                    if(other.floor().hasSurface()){
                        other.setOverlay(Blocks.oreScrap);
                        for(int j = 1; j <= 2; j++){
                            for(var p : Geometry.d8){
                                var t = tiles.get(cx + p.x * j, cy + p.y * j);
        ...
        for(var espawn : enemies){
            tiles.getn(espawn.x, espawn.y).setOverlay(Blocks.spawn);
        }

        state.rules.winWave = sector.info.winWave = 15 + 5 * (int)Math.max(difficulty * 10, 1);

        final float waveTimeDec = 0.4f;

        state.rules.waveSpacing = Mathf.lerp(60.0f * 180.0f, 60.0f * 120.0f, Math.max(difficulty - waveTimeDec, 0.0f));
        state.rules.waves = true;
        state.rules.env = sector.planet.defaultEnv;

        //this doesn't require relevant items when launching but can't be fixed
