...
public class LightningCloudBulletType extends BulletType{
    ...
    private static final short[] sumsOfTime = {30, 150, 300, 330}; //TODO should these be put into type?
    ...
    public LightningCloud create(Entityc owner, Entityc e, Team t, float x, float y, float a, float damage, float v, float l, Object d, Mover m, float p, float q){
        ...
        bullet.time = 0.0f;
        bullet.lifetime = sumsOfTime[sumsOfTime.length - 1];
    ...
    public static class LightningCloud extends Bullet{
        ...
        public void update(){
            ...
            if(time >= sumsOfTime[1] && time < sumsOfTime[2] + 10.0f && Mathf.chanceDelta(type().baseChance + intensity * 0.04f)){
        ...
        public void draw(){
            float alpha;
            if(time <= sumsOfTime[0])
                alpha = time / sumsOfTime[0];
            else if(time <= sumsOfTime[1])
                alpha = 1.0f;
            else
                alpha = 1.0f - (time - sumsOfTime[2]) / sumsOfTime[0];
        ...
        private float size(){
            ...
            if(time <= sumsOfTime[0])
                return base + (time / sumsOfTime[0]) * plus;
            else if(time <= sumsOfTime[2])
                return base + plus + Mathf.sin(time - sumsOfTime[0], scl, mag);
            else
                return (1.0f - (time - sumsOfTime[2]) / sumsOfTime[0]) * (base + plus + Mathf.sin(sumsOfTime[2] - sumsOfTime[0], scl, mag));
        }
